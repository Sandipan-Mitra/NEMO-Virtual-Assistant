...
def MainTaskExecution(query):
    ...
    # task_new = str(query).lower()
    return_data = TasksExecutor(task)
    try:
        if "open" in return_data:
            value = OpenExecute(task)
            return value
            
        elif "time" in return_data:
            speak(check_datetime(task))
            return True
        
        elif "screenshot" in return_data:
        ...
        elif "temperature" in return_data:
            from Features.Temperature import check_temperature
            speak(check_temperature(task))
            return True
            

    except:
        print("Error in tast execution")